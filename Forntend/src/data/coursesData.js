// ==========================================
// COURSE HUB DATA STORE (DATA/courses.json)
// ==========================================

const BACKEND_URL = 'http://localhost:5001';

export const defaultCourses = [
  {
    id: 'course-1',
    title: 'University Admission Physics Masterclass',
    instructor: 'Dr. Rafiqul Islam, BUET (Lead Physics Instructor)',
    rating: 4.95,
    reviewsCount: 140,
    duration: '45 Hours',
    price: 3500,
    discountedPrice: 1800,
    academicGroup: 'Science',
    badge: 'Best Seller',
    image: '⚛️',
    description: 'ঢাবি ক-ইউনিট, বুয়েট ও ইঞ্জিনিয়ারিং গুচ্ছের জন্য পদার্থবিজ্ঞান ১ম ও ২য় পত্রের সকল অধ্যায়ের পূর্ণাঙ্গ প্রস্তুতি।'
  },
  {
    id: 'course-2',
    title: 'Complete Organic Chemistry Mastery',
    instructor: 'Tanvir Ahmed, University of Dhaka',
    rating: 4.9,
    reviewsCount: 115,
    duration: '38 Hours',
    price: 3200,
    discountedPrice: 1600,
    academicGroup: 'Science',
    badge: 'High Rated',
    image: '🧪',
    description: 'জৈব রসায়নের মেকানিজম, বিক্রিয়া এবং রূপান্তর সহজে মনে রাখার আধুনিক শর্টকাট টেকনিক ও প্রশ্ন সমাধান।'
  },
  {
    id: 'course-3',
    title: 'Higher Mathematics Admission Formula & Shortcuts',
    instructor: 'Engr. Shafiul Alam, BUET',
    rating: 4.92,
    reviewsCount: 98,
    duration: '40 Hours',
    price: 3000,
    discountedPrice: 1500,
    academicGroup: 'Science',
    badge: 'Popular',
    image: '📐',
    description: 'ক্যালকুলাস, কনিক, ত্রিকোণমিতি ও জটিল সংখ্যার ক্যালকুলেটর ট্রিকস ও দ্রুততম সমাধান পদ্ধতি।'
  },
  {
    id: 'course-4',
    title: 'Medical Biology Comprehensive Admission Batch',
    instructor: 'Dr. Fariha Tasnim, Dhaka Medical College',
    rating: 4.98,
    reviewsCount: 210,
    duration: '50 Hours',
    price: 3800,
    discountedPrice: 1950,
    academicGroup: 'Science',
    badge: 'Medical Special',
    image: '🧬',
    description: 'মেডিকেল ও ডেন্টাল ভর্তি পরীক্ষার জন্য উদ্ভিদবিজ্ঞান ও প্রাণিবিজ্ঞানের প্রতিটি লাইনের গভীর বিশ্লেষণ।'
  },
  {
    id: 'course-5',
    title: 'University Admission English & Bangla Crash Course',
    instructor: 'Kamrul Hasan, University of Dhaka',
    rating: 4.88,
    reviewsCount: 85,
    duration: '32 Hours',
    price: 2500,
    discountedPrice: 1200,
    academicGroup: 'Arts',
    badge: 'B-Unit Special',
    image: '📖',
    description: 'ঢাবি বি-ইউনিট ও গুচ্ছ মানবিক শাখার জন্য ব্যাকরণ, সাহিত্য ও ভোকাবুলারি স্পেশাল ড্রিল।'
  },
  {
    id: 'course-6',
    title: 'Business Studies Accounting & Management Special Batch',
    instructor: 'Naimul Islam, DU IBA',
    rating: 4.91,
    reviewsCount: 76,
    duration: '35 Hours',
    price: 2800,
    discountedPrice: 1400,
    academicGroup: 'Commerce',
    badge: 'C-Unit Special',
    image: '📊',
    description: 'হিসাববিজ্ঞান ও ব্যবসায় সংগঠনের খুঁটিনাটি ও বিগত ২০ বছরের বিশ্ববিদ্যালয়ের প্রশ্ন সমাধান।'
  }
];

export const mockCourses = defaultCourses;

// Get Courses: Reads from local cache first, triggers async sync from DATA/courses.json
export const getCourses = () => {
  if (typeof window === 'undefined') return defaultCourses;
  try {
    const stored = localStorage.getItem('edufast_dynamic_courses');
    if (stored) {
      const parsed = JSON.parse(stored);
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map(c => {
          const isApproved = c.isApproved !== undefined ? Boolean(c.isApproved) : (c.status === 'Pending' ? false : true);
          return {
            ...c,
            isApproved,
            status: c.status || (isApproved ? 'Approved' : 'Pending'),
            enrolledCount: c.enrolledCount || 0,
            reviewsCount: c.reviewsCount || 0,
            rating: c.rating || 0
          };
        });
      }
    }
  } catch (e) {
    console.error('Error reading dynamic courses from localStorage:', e);
  }

  try {
    localStorage.setItem('edufast_dynamic_courses', JSON.stringify(defaultCourses));
  } catch (e) {}

  return defaultCourses;
};

// Filter only approved courses for public display
export const getPublicCourses = () => {
  return getCourses().filter(c => c.isApproved === true || c.status === 'Approved');
};

// Save course: Updates local cache + syncs to backend DATA/courses.json file
export const saveCourse = (course) => {
  if (typeof window === 'undefined') return defaultCourses;
  const current = getCourses();
  const courseId = course.id || 'course-' + Date.now();
  const formattedCourse = { ...course, id: courseId };

  const index = current.findIndex(c => c.id === courseId);
  let updated;
  if (index >= 0) {
    updated = current.map(c => c.id === courseId ? { ...c, ...formattedCourse } : c);
  } else {
    updated = [formattedCourse, ...current];
  }

  localStorage.setItem('edufast_dynamic_courses', JSON.stringify(updated));
  window.dispatchEvent(new CustomEvent('edufast-data-update', { detail: { type: 'course', action: 'save' } }));

  // Live Sync with backend DATA/courses.json file
  fetch(`${BACKEND_URL}/api/data/courses`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(formattedCourse)
  }).catch(err => console.warn('Backend course sync notice:', err.message));

  return updated;
};

// Delete course: Removes from local cache + deletes from backend DATA/courses.json file
export const deleteCourse = (courseId) => {
  if (typeof window === 'undefined') return [];
  const current = getCourses();
  const updated = current.filter(c => c.id !== courseId);
  localStorage.setItem('edufast_dynamic_courses', JSON.stringify(updated));
  window.dispatchEvent(new CustomEvent('edufast-data-update', { detail: { type: 'course', action: 'delete' } }));

  // Live Delete from backend DATA/courses.json file
  fetch(`${BACKEND_URL}/api/data/courses/${encodeURIComponent(courseId)}`, {
    method: 'DELETE'
  }).catch(err => console.warn('Backend course delete notice:', err.message));

  return updated;
};
